...
test.describe('Customer Browsing Flow', () => {
  test('should display product listings on homepage', async ({ page }) => {
    ...
    await expect(productCards.first().locator('[data-testid=product-image]')).toBeVisible()
    await expect(productCards.first().locator('[data-testid=product-title]')).toBeVisible()
